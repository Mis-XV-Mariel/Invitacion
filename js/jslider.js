...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Función para iniciar el audio en el segundo 20
    function playAudioFromSecond20() {
        if (audio.duration > 20) {
            audio.currentTime = 20;
        }
        audio.play().catch(error => {
            // Manejar el error en caso de que el navegador bloquee la reproducción automática
            console.log('Reproducción automática bloqueada. Requiere interacción del usuario.');
        });
    }

    // Cuando los metadatos del audio están cargados
    audio.addEventListener('loadedmetadata', playAudioFromSecond20);

    // Forzar la reproducción una vez que el navegador haya cargado el audio
    window.addEventListener('load', () => {
        playAudioFromSecond20();
    });

     // Ocultamos el audio para que no sea visible en la página
